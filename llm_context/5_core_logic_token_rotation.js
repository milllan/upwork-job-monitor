...
async function fetchJobsWithTokenRotation(userQuery) {
  ...
  for (const token of candidateTokens) {
    console.log(
      `API: Trying token ${token.substring(0, 15)}... for query: "${userQuery.substring(0, 50)}..."`
    );
    ...
    if (result && !result.error) {
      ...
      if (result && result.graphqlErrors) {
        console.warn(
          `API: GraphQL error with token ${token.substring(0, 15)}... - ${JSON.stringify(result.graphqlErrors)}`
        );
