...
async function runJobCheck(triggeredByUserQuery) {
  ...
  if (fetchedJobs && fetchedJobs.length > 0) {
    ...
    console.log(
      `MV2: Processed ${originalJobCount} jobs. Marked ${titleExcludedCount} as excluded by title. Marked ${skillLowPriorityCount} as low-priority by skill. Marked ${clientCountryLowPriorityCount} as low-priority by client country.`
    );
  }
  ...
  if (fetchedJobs && fetchedJobs.length > 0) {
    ...
    fetchedJobs.forEach((job) => {
      if (job && job.id && !historicalSeenJobIds.has(job.id)) {
        ...
        if (
          (job.isLowPriorityBySkill || job.isLowPriorityByClientCountry) &&
  ...
  console.log(
    `MV2 DirectBG: Token Loop. Found ${allNewOrUpdatedJobs.length} new/updated jobs, ${notifiableNewJobs.length} are notifiable.`
  );
  ...
  await StorageManager.setRecentFoundJobs(
    fetchedJobs ? fetchedJobs.filter((job) => job && job.id && !deletedJobIds.has(job.id)) : []
  );
  ...
  try {
    ...
    console.warn('MV2: Error sending updatePopupDisplay message post-check:', e);
  }
}
